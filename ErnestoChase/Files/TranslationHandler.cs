...
{
	private static Dictionary<TextTranslation.Language, Dictionary<string, string>> _shipLogTranslationDictionary =
		new();

	private static Dictionary<TextTranslation.Language, Dictionary<string, string>> _dialogueTranslationDictionary =
		new();
	...
	public static string GetTranslation(string text, TextType type, bool warn)
	{
		...
		switch (type)
		{
			...
			case TextType.OTHER:
				...
				break;
			default:
				if (warn) ErnestoChase.WriteDebugMessage($"Invalid TextType {type}");
				return text;
		}
		...
		if (warn)
		{
			//ErnestoChase.WriteDebugMessage($"Defaulting to english for {text}");
		}
	...
	private static bool TryGetTranslatedText(Dictionary<Language, Dictionary<string, string>> dict, Language language,
		string text, out string translatedText)
	{
